  ...
  if (ret < 0)
    {
      ...
      /* Known limitation: neither esp_i2cbus_bitbang_* nor the generic
       * i2c_bitbang driver provides an uninitialize API, so the interface
       * allocated above cannot be released here.  This boot-time error
       * path therefore leaks the bitbang instance; adding an uninitialize
       * API upstream is tracked as follow-up work.
       */
    }
