...
int esp_bringup(void)
{
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: board_rmt_rxinitialize() failed: %d\n", ret);
    }
#endif
...
#    ifdef CONFIG_SPI_DRIVER
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: Failed to init bitbang spidev: %d\n", ret);
    }
#    endif
...
#  endif
  ...
  if (dev == NULL)
    {
      syslog(LOG_ERR, "Failed to initialize SDM DAC device\n");
    }
  else
    {
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR, "Failed to initialize GT9xx touch panel: %d\n", ret);
    }
