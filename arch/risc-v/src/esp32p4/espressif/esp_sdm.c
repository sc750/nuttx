/****************************************************************************
 * arch/risc-v/src/esp32p4/espressif/esp_sdm.c
 *
  ...
  struct esp_sdm_chan_config_s config, int group_id, int chan_id)
{
  ...
  gpio_ll_iomux_set_clk_src(g_esp_sdm.groups[ret->group_id]->clk_src);
#endif
...
struct dac_dev_s *esp_sdminitialize(struct esp_sdm_chan_config_s config)
{
  ...
  if (ret == NULL || sdm == NULL)
    {
      aerr("Error! %s\n",
           ret == NULL ? "Not enough memory" : "SDM group init failed");
      if (sdm != NULL)
        {
          free(sdm);
        }
